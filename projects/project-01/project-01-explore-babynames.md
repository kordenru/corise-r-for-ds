...
tbl_names_popular |> 
  ...
  labs(
    title = 'bar plot of number of births by name faceted by sex',
    subtitle = '___',
    caption = 'data from year 2011-2020',
    x = 'Number of births',
    y = 'Names'
  ) +
...
tbl_names_by_letter |> 
  ...
  # Add labels (title, subtitle, x, y)
      labs(title = "Distribution of births by first letter")+
      labs(caption = "Year:2020")+
      labs(x = "First letters of name")+
      labs(y = "Percentage")
```
...
``` r
# Analyzing LAST LETTER of every name / BONUS
# # Creating data table
tbl_names_by_letter = tbl_names |> 
  # Group by year, sex and first_letter
...
``` r
#Creating and visualizing distribution of last letters.
...
tbl_names_by_letter |> 
  ...
  # Facet wrap plot by sex
      facet_wrap(~sex)+
  # Add labels (title, subtitle, x, y)
      labs(title = "Distribution of births by last letter")+
      labs(caption = "Year:2020")+
      labs(x = "Last letters of name")+
      labs(y = "Percentage")
```
...
``` r
# Creating Trend line for names ending with letter 'A' over the years
...
<img src="img/unnamed-chunk-1-2.png" width="100%" style="display: block; margin: auto;" />
...
tbl_names_by_first_and_last_letter |> 
  ...
  labs(title = "Heatmap of the percentage of births by first letter and last letter")+
  labs(x = "First letter")+
  labs(y = "Last letter")+
  labs(caption = 'Year 2021')+
...
tbl_names_vowel_consonant <- tbl_names |> 
  ...
  # Add NEW column with `pct_births` calculated as `nb_births / sum(nb_births)`
   mutate(pct_births = (nb_births/sum(nb_births))*100) |>
  # Ungroup the data
  ungroup() |>
...
```
    ...
    #>    <chr> <dbl> <chr>                     <dbl>      <dbl>
    #>  1 F      1880 consonant | consonant     19988      22.0 
    #>  2 F      1880 consonant | vowel         46765      51.4 
    #>  3 F      1880 vowel | consonant          5708       6.27
    #>  4 F      1880 vowel | vowel             18533      20.4 
    #>  5 F      1881 consonant | consonant     20069      21.8 
    #>  6 F      1881 consonant | vowel         47287      51.4 
    #>  7 F      1881 vowel | consonant          5669       6.17
    #>  8 F      1881 vowel | vowel             18928      20.6 
    #>  9 F      1882 consonant | consonant     23561      21.8 
    #> 10 F      1882 consonant | vowel         55449      51.4 
    #> # ℹ 1,126 more rows
...
tbl_names_vowel_consonant |> 
  ...
  ggplot(aes(y= pct_births, x= year, fill = first_last))+
  # Add an area layer with fill = first_last
  geom_area()+
  # Facet wrap plot by `sex`
  facet_wrap(~sex)+
  # Add labels (title, subtitle, caption, x, y)
  labs(title = "The percentage of births by the combination of first and last letter types")+
